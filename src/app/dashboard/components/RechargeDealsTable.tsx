...
function RechargeDealsTable({ onRegisterExport }: { onRegisterExport?: (fn: () => void) => void }) {
  ...
  const filteredTransactions = useMemo(() => {
    return transactions?.filter((transaction: { id: string | number; name: string; email: string; item: any; date: string | string[]; deviceId: string; biller?: string; }) => {
      const matchesSearch =
  ...
  }, [transactions, search, billerFilter, selectedDate]);
                ...
                <td className="px-6 py-4">
                  ...
                  >
                    ...
                    {menuOpen === transaction.id && (
                      <div className="absolute px-4 right-6 bg-white border border-gray-200 rounded-lg shadow-lg z-10 py-2 min-w-[150px]">
                        <button
                          ...
                          onClick={() => {
                            setMenuOpen(null);
                            router.push(`/dashboard/rechargedeals/${transaction.id}`);
                          }}
                        >
                          View details
                        </button>
