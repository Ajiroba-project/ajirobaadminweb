...
function RegularsDealTable() {
  ...
  const transactions = transInfo && transInfo?.data?.map((order: { order_id: any; name: any; email: any; products: any[]; date_created: string | number | Date; profile_image: any; amount: any }, index: number) => ({
   id: order?.order_id,
  // id: index + 1,
  name: order.name,
  email: order.email,
