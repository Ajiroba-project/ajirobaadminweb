...
function RegularsDealTable({ onRegisterExport }: { onRegisterExport?: (fn: () => void) => void }) {
  ...
  const transactions = transInfo && transInfo?.data?.map((order: { order_id: any; status: any; name: any; email: any; products: any[]; date_created: string | number | Date; profile_image: any; amount: any }, index: number) => ({
    ...
    item: typeof order.products?.[0] === 'string'
      ? order.products[0]
  ...
  // Register CSV exporter with parent using latest data via ref to avoid effect loops
  const exportColumns = useMemo(() => ([
    { key: 'name', header: 'Name' },
    { key: 'email', header: 'Email' },
    { key: 'amount', header: 'Ticket Amount' },
  ...
  useEffect(() => {
    ...
    onRegisterExport(() => exportToCSV(latestDataRef.current, { fileName: 'Regular_Transactions', columns: exportColumns }));
    // onRegisterExport is provided by parent and memoized; exportColumns is stable
        ...
        <table className="w-full table-auto">
          <thead className="bg-[#F9FAFB] border-b border-gray-200">
            <tr className="text-[#344054] font-Poppins font-medium text-sm">
              ...
              <th className="px-6 py-4 text-left">Ticket Amount</th>
                ...
                <td className="px-6 py-4">
                  ...
                  >
                    ...
                    {menuOpen === transaction.id && (
                      <div className="absolute right-6 bg-white border border-gray-200 rounded-lg shadow-lg z-10 py-2 min-w-[150px]">
                        <button
                          ...
                          onClick={() => {
                            setMenuOpen(null);
                            router.push(`/dashboard/regulardeals/${transaction.id}`);
                          }}
                        >
                          View details
                        </button>
                      {/*   <button
                          className="w-full px-4 py-2 text-left text-sm bg-[#E84526] rounded-lg text-white hover:bg-[#D63A1F] font-Poppins font-medium transition-colors duration-200 mt-2"
                          onClick={() => handleConfirmDelivery(transaction)}
                        >
                          Confirm Delivery
                        </button> */}
                        <button
                          className="w-full px-4 py-2 text-left text-sm bg-white border border-[#E84526] rounded-lg text-[#E84526] hover:bg-gray-50 font-Poppins font-medium transition-colors duration-200 mt-2"
